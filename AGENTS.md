...
**TTS reliability rule:** When a TTS request fails on the first method, immediately try the next installed TTS path before telling the user it is unavailable. Do fallback discovery proactively instead of assuming misconfiguration.
...
- If no current path works, search the web for implementation methods, prioritizing free or locally runnable options before paid services.
